...
<?php 
    ...
    // NOTE: Avoid ORDER BY RAND() (slow/unreliable at scale). Instead select IDs and shuffle in PHP.
    $stmt = $conn->prepare("SELECT eqt_id FROM exam_question_tbl WHERE exam_id = ? AND exam_status = 'active'");
    $stmt->execute([(int)$examId]);
    $questionIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

    if (empty($questionIds)) {
        die("No active questions found for this exam");
    }

    shuffle($questionIds);
    $limit = (int)$exDisplayLimit;
    if ($limit > 0) {
        $questionIds = array_slice($questionIds, 0, $limit);
    }

    // Fetch selected questions (keep same order as shuffled IDs)
    $placeholders = implode(',', array_fill(0, count($questionIds), '?'));
    $stmt = $conn->prepare("SELECT * FROM exam_question_tbl WHERE eqt_id IN ($placeholders)");
    $stmt->execute($questionIds);
    $questionsById = [];
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $questionsById[(int)$row['eqt_id']] = $row;
    }
...
function selectChoice(questionNum, letter, value) {
    // Find the target input by its stable id (avoids selector issues when value contains quotes)
    const questionId = $(`#question_${questionNum} input[type="radio"]`).first().attr('name').match(/\[(\d+)\]/)[1];
    const inputId = `q${questionId}_${letter}`;

    // Update visual selection
    $(`#question_${questionNum} .choice-item`).removeClass('selected');
    $(`#question_${questionNum} #${inputId}`).closest('.choice-item').addClass('selected');

    // Check the radio button
    $(`#question_${questionNum} #${inputId}`).prop('checked', true);
